...
| Specifier FunDec CompSt {$2->t=fun;def($1,$2,imap);eq($2,$3,checkreturn,imap);}
...
| VarDec LB INT RB {$1->t=arr; $1->val=new rec(noact); buildarr($1->val,$3);}
...
FunDec:
  ID LP VarList RP {$1->line_num=@1.last_line;$1->recs=$3->recs;$$=$1;}
| ID LP VarList error {syntax_error("closing parenthesis \')\'",@3.last_line);}
| ID LP RP {$1->line_num=@1.last_line;$$=$1;}
...
Dec:
  VarDec {$$=$1;}
| VarDec ASSIGN Exp {eq($1,$3,usassign,imap);}
//检查两边变量
...
Exp:
  Exp ASSIGN Exp {eq($1,$3,usassign,imap);}
  //var 需通过val检测，这个val的t将表示它的类型，val将指向它所存的值
  //检查两边变量，尝试另var val的val指向EXP
| Exp AND Exp {eq($1,$3,usbiop,imap);}
| Exp OR Exp {eq($1,$3,usbiop,imap);}
| Exp LT Exp {eq($1,$3,usbiop,imap);}
| Exp LE Exp {eq($1,$3,usbiop,imap);}
| Exp GT Exp {eq($1,$3,usbiop,imap);}
| Exp GE Exp {eq($1,$3,usbiop,imap);}
| Exp NE Exp {eq($1,$3,usbiop,imap);}
| Exp EQ Exp {eq($1,$3,usbiop,imap);}
| Exp PLUS Exp {eq($1,$3,usbiop,imap);}
| Exp MINUS Exp {eq($1,$3,usbiop,imap);}
| Exp MUL Exp {eq($1,$3,usbiop,imap);}
| Exp DIV Exp {eq($1,$3,usbiop,imap);}
...
//该节点val设为其第一个变量类型的复制
| ID LP Args RP {$1->line_num=@1.last_line;usfun($1,$3,imap);}
